...
export default function Results() {
  ...
  useEffect(() => { if (selected) fetchResults(selected._id || selected.id || selected); }, [selected]);
  ...
  const fetchResults = async (electionId) => {
    try {
      ...
      setResults({ candidates: [], totalVotes: 0, totalVoters: 0, error: "Failed to load results." });
  ...
  // Function to determine winner's color
  const getCandidateBarColor = (index) => {
    const colors = ['bg-sky-500', 'bg-purple-500', 'bg-emerald-500', 'bg-yellow-500'];
    return colors[index % colors.length];
  }

  // Find the winning candidate (assuming highest vote count wins)
  const winner = results && results.candidates.reduce((prev, current) => 
      ...
      <div className="max-w-4xl mx-auto text-white p-4">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 border-b border-gray-700/50 pb-4">
          ...
          <select 
            ...
            className="bg-gray-800 border border-gray-700 text-gray-200 px-4 py-2 rounded-xl shadow-md focus:ring-sky-500 focus:border-sky-500 transition duration-150 mt-3 sm:mt-0"
          >
        ...
        {results && !loading ? (
          <div className="space-y-8">
            ...
            <div className="bg-gray-800 border border-gray-700 rounded-xl p-6 shadow-lg flex justify-around text-center">
                <div>
                    <div className="text-4xl font-extrabold text-sky-400">{results.totalVotes || 0}</div>
                    <div className="text-gray-400 text-sm mt-1">Total Votes Cast</div>
                </div>
                <div>
                    <div className="text-4xl font-extrabold text-green-400">{results.totalVoters || 0}</div>
            ...
            {winner && winner.votes > 0 && !isTie && (
                ...
                >
                    <h3 className="text-2xl font-extrabold text-emerald-300">🎉 Current Winner: {winner.name}</h3>
                    <p className="text-sm text-emerald-400 mt-1">Leading with {winner.votes} votes.</p>
                </motion.div>
            )}
            
            {/* Candidate Results List */}
            <div className="space-y-5">
              {(results.candidates || []).sort((a, b) => b.votes - a.votes).map((c, index) => {
                const pct = results.totalVotes > 0 ? (c.votes / results.totalVotes) * 100 : 0;
                const isLeading = c.votes === winner?.votes && !isTie;
                const barColor = isLeading ? 'bg-sky-500 shadow-sky-800' : getCandidateBarColor(index);
                
                return (
                  <motion.div 
                    key={c._id || c.id} 
                    className={`bg-gray-800 p-4 rounded-xl border ${isLeading ? 'border-sky-500 shadow-lg' : 'border-gray-700'}`}
                    initial={{ x: -20, opacity: 0 }}
                    animate={{ x: 0, opacity: 1 }}
                    transition={{ delay: index * 0.1 }}
                  >
                    <div className="flex justify-between items-center mb-2">
                      <div className={`font-extrabold text-lg ${isLeading ? 'text-sky-300' : 'text-white'}`}>
                        {isLeading && '🏆 '} {c.name}
                      </div>
                      <div className="text-sm text-gray-400 font-mono">
                        {c.votes} votes · <span className="text-white font-semibold">{pct.toFixed(1)}%</span>
                      </div>
                    </div>
          ...
          !loading && <p className="text-gray-400 text-lg py-8 text-center bg-gray-800/50 rounded-xl">Select an election or wait for votes to be cast.</p>
        )}
      </div>
    </DashboardLayout>
  );
